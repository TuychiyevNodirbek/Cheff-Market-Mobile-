...
fun SharedPreferences.saveAddresses(addresses: List<SavedAddress>, activeId: String?) {
    edit {
        putString(
            KEY_ADDRESSES,
...
fun SharedPreferences.loadAddresses(): Pair<List<SavedAddress>, String?> {
    ...
    return addresses to getString(KEY_ACTIVE_ADDRESS, null)
}

fun SharedPreferences.saveLastGpsPoint(lat: Double, lon: Double) {
    edit {
...
fun SharedPreferences.loadLastGpsPoint(): Pair<Double, Double>? {
    val lat = getString(KEY_GPS_LAT, null)?.toDoubleOrNull() ?: return null
    val lon = getString(KEY_GPS_LON, null)?.toDoubleOrNull() ?: return null
    return lat to lon
}
